...
l=Math.max(l,r.args[0].prop("scrollHeight"))}}0!=w&&(m=0);h[2]={badness:b-k,count_hss:w,count_hskip:m};n+=l}else switch(h[1].command){case "vss":a++;break;case "vskip":e++;n+=h[1].args[0];break;case "vkern":n+=h[1].args[0]}}0!=a&&(e=0);b=f-n;for(n=f=0;n<c.length;n++)if(g=c[n],0==g[0])for(d=0,h=g[1],g=g[2],k=0;k<h.length;k++)switch(l=h[k],l.command){case "hss":d+=g.badness/g.count_hss;break;case "hskip":0!=g.count_hskip&&(d+=g.badness/g.count_hskip);d+=l.args[0];break;case "hkern":d+=l.args[0];break;
case "box":l.args[0].attr({x:d,y:f+l.args[0].prop("scrollHeight")}),d+=l.args[0].prop("scrollWidth")}else switch(g[1].command){case "vss":f+=b/a;break;case "vskip":0!=e&&(f+=b/e);f+=g[1].args[0];break;case "vkern":f+=g[1].args[0]}return this}},m=function(b){return{width:b.width(),height:b.height()}},l=function(b){this._=document.createElementNS("http://www.w3.org/2000/svg",b);this.attr=function(a){var b=this._;Object.keys(a).forEach(function(f){b.setAttribute(f,a[f])});return this};this.css=function(a){var b=
