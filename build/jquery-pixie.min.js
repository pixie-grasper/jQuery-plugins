...
color:"lightgray"},c=function(a){for(var l=0;l<a.length;l++)a[l].$.detach()},n=function(a,b){"string"===typeof a.pos&&a.pos.match(/%/)?a.true_pos=eval(a.pos.replace(/%/," / 100 * "+b)):a.true_pos=a.pos;"string"===typeof a.pos_min&&a.pos_min.match(/%/)?a.true_pos_min=eval(a.pos_min.replace(/%/," / 100 * "+b)):a.true_pos_min=a.pos_min;"string"===typeof a.pos_max&&a.pos_max.match(/%/)?a.true_pos_max=eval(a.pos_max.replace(/%/," / 100 * "+b)):a.true_pos_max=a.pos_max},m=function(a){return{width:a.width(),
height:a.height()}},p=function(){this.bar={$:b("<div></div>").css({backgroundColor:this.options.color,position:"absolute"})};for(var a=0;a<this.children.length;a++)this.children[a].$.css({position:"absolute"})},u=function(){var a=this.$.height();this.bar.$.css({cursor:"col-resize",width:this.options.border,height:a});for(var b=0;b<this.children.length;b++)this.children[b].$.css({height:a})},w=function(){u.call(this);var a=!1,f=this;this.bar.$.mousedown(function(l){f.drag_start_pos_x=l.clientX;f.options_pos_init=
...
[],c=0;c<arguments.length;++c)a[c-0]=arguments[c];return v.initialize.apply(this,a)}}()};b.fn.pixie=function(c,k){for(var e=[],m=1;m<arguments.length;++m)e[m-1]=arguments[m];if(g[c])return g[c].apply(this,e);b.error("Widget named "+c+" not found.")};b(function(){b(".pixie-widget").hide()})})(jQuery);
